...
这样设计的好处是，`<think>` 是所有 RL 能力域共享的输出协议，而不是某个任务 prompt 的一部分。Temporal、Static Spatial 和 Spatiotemporal Tracking 的 user prompt 只负责描述任务和最终答案格式；格式纪律由 system prompt 统一约束。Separate RL、Mixed-RL 和 MOPD 必须使用完全相同的 system prompt，否则 prompt 差异会混进优化范式差异里。
...
Static Spatial Teacher 学框定位，输入是图片和类别名或 referring expression，输出一个归一化框：

```python
assistant_content = "<answer>[0.12, 0.35, 0.48, 0.77]</answer>"
```

Spatiotemporal Tracking Teacher 学跨帧轨迹，输入是视频或 8 帧序列、对象名和初始框，输出 8 个归一化框：
...
这三个输出字段就是三个能力域的边界。Temporal 的 reward 看 temporal IoU，Static Spatial 的 reward 看 box IoU / grounding accuracy，Spatiotemporal 的 reward 看 trajectory average IoU。是否写 `<think>` 不是能力域定义的核心；核心是最终可验证字段。

## Separate SFT

Separate SFT 是分别训练三个 SFT checkpoint，作为后续 RL 的初始化或对照。这里不需要 `<think>`，因为旧数据集没有真实思考链。SFT 的目标应该是学会任务格式和可验证答案，不应该监督模型编造 reasoning。
...
messages = [
        ...
        "content": [
            {
                "type": "text",
                "text": (
                    'Locate the object or region described by "the man holding a red cup" '
                    "in the image.\n\n"
                    "Provide the bounding box as four normalized coordinates "
                    "[x0, y0, x1, y1] within the <answer> </answer> tags."
                ),
            },
            {
                "type": "image",
    ...
    {
        "role": "assistant",
        "content": "<answer>[0.12, 0.35, 0.48, 0.77]</answer>",
    },
]
```

Spatiotemporal Tracking SFT:
...
Separate RL 是分别训练三个能力教师：Temporal RL Teacher、Static Spatial RL Teacher、Spatiotemporal RL Teacher。和 SFT 不同，RL 阶段没有 assistant label；模型在线采样 completion，reward 从 completion 中解析 `<answer>`。
...
Static Spatial reward:

```python
reward = format_reward(completion) + box_iou_reward(completion, gt_box)
```

Spatiotemporal Tracking RL messages:
...
Mixed-RL 是把三个能力域的数据混在一起，训练一个总模型。它不是 MOPD，因为没有冻结教师，也没有 teacher log-prob 蒸馏；每条样本仍然只使用自己的 reward。
...
elif example["data_type"] == "static_spatial":
    messages = build_static_spatial_rl_messages(example)
    reward = spatial_format_reward + box_iou_reward
elif example["data_type"] == "spatiotemporal":
    messages = build_tracking_rl_messages(example)
    reward = tracking_format_reward + trajectory_average_iou_reward
```

Mixed-RL 的核心控制变量是采样比例，而不是 prompt。第一版应该固定每个能力域的 sample budget 或 token budget；否则 COCO / RefCOCO 的数据量会支配训练，使结果变成数据量效应。
...
if example["data_type"] == "temporal":
    ...
    teacher = static_spatial_rl_teacher
elif example["data_type"] == "spatiotemporal":
    teacher = spatiotemporal_rl_teacher
```

这也是为什么 MOPD 里的教师不是数据集教师。`Charades` 和 `ActivityNet` 只是 Temporal domain 的数据来源；`COCO` 和 `RefCOCO` 只是 Static Spatial domain 的数据来源；`GoT` 只是 Spatiotemporal Tracking domain 的数据来源。MOPD 的路由单位应该是 capability domain，而不是 dataset name。
...
Mixed-RL:
  ...
  all domains mixed in one dataloader
